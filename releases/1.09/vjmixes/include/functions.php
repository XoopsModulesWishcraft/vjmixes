<?php
	...
	function saveEditVideo($video_id)
	{
		...
		$video->setVar('video_tags', $_REQUEST["video_tags"]);
		$video->setVar('embedded', $_REQUEST["embedded"]);
		...
		if ($videoHandler->insert($video)) {
			...
			if (!strpos($_SERVER['REQUEST_URI'], 'vjmixes/admin/'))
				redirect_header('admin.php?op=edit&id='.$video_id, 8, sprintf(_VJM_RH_VJMIXES_EDITED, $video->getVar('name')));
			else
				redirect_header('admin.php?op=list', 8, sprintf(_VJM_RH_VJMIXES_EDITED, $video->getVar('name')));
			exit(0);	
		} else {
			redirect_header('index.php', 4, sprintf(_VJM_RH_VJMIXES_NOCREATION, $video->getVar('name')));
			exit(0);		
		}
	}
	...
	function saveNewVideo() {
		...
		if ($videoHandler->insert($video)) {
			...
			if (!strpos($_SERVER['REQUEST_URI'], 'vjmixes/admin/'))
				redirect_header('admin.php?id='.$video_id, 8, sprintf(_VJM_RH_VJMIXES_EDITED, $video->getVar('name')));
			else
				redirect_header('admin.php?op=list', 8, sprintf(_VJM_RH_VJMIXES_EDITED, $video->getVar('name')));
			exit(0);	
		} else {
	...
	function saveEditVideoCategory($cid)
	{
		...
		if ($cid = $videoCatHandler->insert($video)) {				
			if (!strpos($_SERVER['REQUEST_URI'], '/vjmixes/admin/'))
				redirect_header('admin.php?op=cats&fct=edit&id='.$cid, 8, sprintf(_VJM_RH_CATEGORY_EDITED, $video->getVar('name')));
			else
				redirect_header('admin.php?op=list&fct=cats', 8, sprintf(_VJM_RH_CATEGORY_EDITED, $video->getVar('name')));
			exit(0);	
		} else {
			redirect_header('index.php', 4, sprintf(_VJM_RH_CATEGORY_NOCREATION, $video->getVar('name')));
			exit(0);		
		}
	}
